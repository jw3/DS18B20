...
object Monitor {
  def apply(relay: ActorRef, cfg: Config)(implicit sys: ActorSystem) = sys.actorOf(Props(new Monitor(relay, cfg)))

  case class Configure(config: Config)
}

// default cfg is passed in ctor, available if reset is needed due to invalid settings
class Monitor(relay: ActorRef, cfg: Config)(implicit sys: ActorSystem) extends Actor with ActorLogging {
  val history = History()
